...
function App() {
  ...
  const [selectedLevels, setSelectedLevels] = useState<LearningLevel[]>([]);
  ...
  const filteredResources = useMemo(() => {
    return resources.filter((resource) => {
      ...
      const matchesLevel =
        selectedLevels.length === 0 || selectedLevels.includes(resource.level);
  ...
  const handleLevelToggle = (level: LearningLevel) => {
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 font-heading">Blockchain Learning Hub</h1>
              <p className="text-sm text-gray-600 mt-0.5">
                Your comprehensive guide to blockchain development
              </p>
            </div>
          </div>
        </div>
      ...
      <footer className="bg-white border-t border-gray-100 mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center text-sm text-gray-600">
            <p className="mb-2">
              A curated collection of blockchain learning resources for developers at all levels
            </p>
            <p className="text-gray-500">
              Free and open for the community. Happy learning!
            </p>
          </div>
        </div>
      </footer>
